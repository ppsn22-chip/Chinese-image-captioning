...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', '-m', default='models/BEST_checkpoint_11.pth.tar', help='path to model')
    parser.add_argument('--word_map', '-wm', default='data/WORDMAP.json', help='path to word map JSON')
    parser.add_argument('--beam_size', '-b', default=5, type=int, help='beam size for beam search')
    parser.add_argument('--dont_smooth', dest='smooth', action='store_false', help='do not smooth alpha overlay')
    parser.add_argument('--new_path',type=str,default='../predict_result/share_path.json',help='store the new image path')
    ...
    while(True):
        ...
        img_path = get_image_path(args.new_path)
        print("image_path",img_path)

        # Encode, decode with attention and beam search
        seq, alphas = caption_image_beam_search(encoder, decoder, img_path, word_map, args.beam_size)
        alphas = torch.FloatTensor(alphas)
        # FIXME seq is the result 
        # print(seq)
        words = [rev_word_map[ind] for ind in seq]

        sentence = "".join(words[1:-1])
        # print(words)
        # item = {img_path:{"CN_SoftAtt":sentence}}
        content = {}
        with open("../predict_result/predict.json",'r+') as predict_file:
            if bool(predict_file.readline()):   # 文件非空才读
                predict_file.seek(0)
                content = json.load(predict_file)
            predict_file.seek(0)
            #FIXME 此处有存储时间长的危险，需要根据需求来改进
            if not img_path in content:
                content[img_path] = {}
            content[img_path]['CN_SoftAtt'] = sentence
            
            json.dump(content,predict_file)
